...
const server=http.createServer(async(req,res)=>{
 ...
 if(p==='/api/stats'){if(!authed(req))return json(res,{error:'Não autorizado'},401);const d=data(),cut=Date.now()-30*86400000,recent=a=>a.filter(x=>new Date(x.at).getTime()>=cut),visits=recent(d.visits),clicks=recent(d.clicks),leads=recent(d.leads),daily={};for(let i=29;i>=0;i--){const k=new Date(Date.now()-i*86400000).toISOString().slice(0,10);daily[k]={visits:0,clicks:0,leads:0}}visits.forEach(x=>daily[x.at.slice(0,10)]?.visits++);clicks.forEach(x=>daily[x.at.slice(0,10)]?.clicks++);leads.forEach(x=>daily[x.at.slice(0,10)]?.leads++);const products={},segments={};leads.forEach(x=>{products[x.produto]=(products[x.produto]||0)+1;segments[x.segmento]=(segments[x.segmento]||0)+1});return json(res,{totals:{visits:d.visits.length,clicks:d.clicks.length,leads:d.leads.length},last30:{visits:visits.length,clicks:clicks.length,leads:leads.length},daily,products,segments,leads:leads.slice(-50).reverse()})}
 if(p==='/'||p==='/admin.html'){const file=path.join(__dirname,'public/admin.html');if(fs.existsSync(file)){res.writeHead(200,{'Content-Type':'text/html; charset=utf-8'});return fs.createReadStream(file).pipe(res)}}
